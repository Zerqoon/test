...
import {profileCard,rankCard,economyCard,topCard,welcomeCard,shopCard,inventoryCard,bankCard,achievementsCard,rewardCard} from '../src/cards.js';
...
const renders=[
 ...
 ['15-praca-wyplata.png',()=>rewardCard(demo,row,{kind:'work',baseReward:184,reward:276,balance:48526,availableAt:now+2700000,job:'Przygotowano zestaw na event.'})],
 ['16-daily-wyplata.png',()=>rewardCard(demo,row,{kind:'daily',baseReward:350,reward:600,balance:49126,availableAt:now+86400000,streak:6})],
 ['17-weekly-wyplata.png',()=>rewardCard(demo,row,{kind:'weekly',baseReward:1200,reward:1200,balance:50326,availableAt:now+604800000})],
];
for(const [name,render] of renders){fs.writeFileSync(path.join(folder,name),await render());console.log(`Podgląd: previews/${name}`);}
fs.writeFileSync(path.join(folder,'README.md'),'# Podglądy Wymiaru ZERQONA v4\n\nDane w przykładach są demonstracyjne. Bot generuje karty z aktualnej bazy i awatarów Discord. Pliki 15–17 pokazują publiczne karty wypłat dla /praca, /daily i /weekly.\n');
