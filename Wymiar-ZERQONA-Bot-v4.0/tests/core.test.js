...
function fixture(t){
  const dir=fs.mkdtempSync(path.join(os.tmpdir(),'zerqona-test-'));
...
test('Poziomy mają poprawny próg i nagradzają tylko nowe poziomy',t=>{
  ...
  const result=db.grantMessageXp('guild','alice','awans',need,{messageCooldownMs:45000,repeatCooldownMs:300000},1_000_000);
...
test('Karty graficzne zwracają prawidłowe PNG, komendy mają unikalne nazwy',async()=>{
  ...
  assert.equal(commandPayload.length,52);
  assert.equal(new Set(commandPayload.map(c=>c.name)).size,commandPayload.length);
  assert.ok(commandPayload.some(c=>c.name==='komendy'));
  for(const item of Object.values(items).filter(i=>i.type==='consumable')) assert.ok(item.durationMs>0, `${item.name} musi być czasowy`);
});
