...
const colors={message:0xA78BFA,voice:0x8D73BC,account:0xD8CAFF,change:0xB59BD3};
...
async function send(guild,kind,title,fields,description='',mention=null,image=null){
  ...
  const embed=new EmbedBuilder().setAuthor({name:BRAND.name}).setFooter({text:`${BRAND.name} • Dziennik zdarzeń`}).setColor(colors[kind]).setTitle(title).setDescription(description||null)
