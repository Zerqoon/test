...
commands.push(
  ...
  new SlashCommandBuilder().setName('ekonomia').setDescription(commandMeta.ekonomia.summary).addUserOption(o=>o.setName('osoba').setDescription('Osoba (domyślnie Ty)')),
  ...
  new SlashCommandBuilder().setName('komendy').setDescription(commandMeta.komendy.summary)
    .addSubcommand(s=>s.setName('status').setDescription('Porównaj bieżące komendy z tym, co jest zapisane w Discordzie'))
    .addSubcommand(s=>s.setName('synchronizuj').setDescription('Zastąp listę komend serwera aktualnym zestawem'))
    .addSubcommand(s=>s.setName('wyczysc_stare').setDescription('Usuń globalne duchy i zsynchronizuj komendy serwera')),
);
for (const command of commands) {
  ...
  if (['profil','ranga','portfel','top','userinfo','avatar','ekonomia','nagrody'].includes(command.name))
...
export async function handleCommand(i,store){
  ...
  if(await handleAdmin(i,store,commandPayload))return;
