...
export async function shopPayload(i,store,id='xp_boost',category='all',page=1){
  ...
  const card=embed(`Sklep • ${item.name}`,`${item.description}\n\n**Zasady:** ${itemRules(item)}`).setImage('attachment://zerqona-sklep.png')
    .addFields({name:'Cena / saldo',value:`${fmt(item.price)} / ${fmt(row.balance)} ◈`,inline:true},{name:'Jakość / czas',value:`${rarityNames[item.rarity]} • ${item.durationMs?duration(item.durationMs/1000):'na stałe'}`,inline:true});
...
export function purchase(store,guild,user,id){
  ...
  if(!store.buy(guild,user,id))return {ok:false,message:`Potrzebujesz **${fmt(item.price)} ◈**. Masz **${fmt(store.user(guild,user).balance)} ◈**.`};
  return {ok:true,message:`**${item.name}** trafia do plecaka.\nKoszt: **${fmt(item.price)} ◈** • saldo: **${fmt(store.user(guild,user).balance)} ◈**.\nAktywuj przez \`/uzyj przedmiot:${id}\`.`};
}
...
export async function showInventory(i,store){
  ...
  const image=await inventoryCard(userForCard(i.member),row,inventory),select=new StringSelectMenuBuilder().setCustomId(`wz:inventory:${i.user.id}`).setPlaceholder('Załóż lub aktywuj przedmiot')
    .addOptions(inventory.filter(r=>items[r.item_id]).map(r=>({label:items[r.item_id].name,value:r.item_id,description:permanent(items[r.item_id])?'Stały przedmiot':`${r.quantity} sztuk`})).slice(0,25));
...
export async function handlePublicComponent(i,store){
  ...
  if(['buy','use','inventory','achclaim'].includes(action)){
    ...
    if(action==='achclaim'){
      const claimed=store.claimAchievements(i.guildId,i.user.id);result={ok:claimed.count>0,message:claimed.count?`Odebrano **${claimed.count}** osiągnięć: **+${fmt(claimed.reward)} ◈**.`:'Nie masz nowych nagród do odebrania.'};await showAchievements(i,store);
