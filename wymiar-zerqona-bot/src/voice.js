...
import { BRAND, denyPermission } from './ui.js';
...
function panel(room,channel){
  ...
  return {
    embeds:[new EmbedBuilder().setAuthor({name:BRAND.name}).setColor(BRAND.color).setTitle('Twój pokój • panel sterowania')
      ...
      .setFooter({text:'Wymiar ZERQONA • /pokoj • pusty kanał znika po 30 s'})],
    components:[
...
async function createRoom(member,store){
  ...
  if(existing){
    const channel=guild.channels.cache.get(existing.channel_id)||await guild.channels.fetch(existing.channel_id).catch(e=>{if(e.code===10003)return null;throw e;});
  ...
  // Keep the bot reachable when @everyone is hidden or locked later.
  const botId=guild.members.me.id;
  const botFlags=PermissionFlagsBits.ViewChannel|PermissionFlagsBits.Connect|PermissionFlagsBits.SendMessages|PermissionFlagsBits.ReadMessageHistory|PermissionFlagsBits.EmbedLinks|PermissionFlagsBits.AttachFiles;
  const botOverwrite=overwrites.find(o=>o.id===botId);
  if(botOverwrite){botOverwrite.allow=BigInt(botOverwrite.allow)|botFlags;botOverwrite.deny=BigInt(botOverwrite.deny)&~botFlags;}
  else overwrites.push({id:botId,allow:botFlags});
  let channel;
  try{
...
export function scheduleRoomCleanup(channelId,client,store,delay=30000){
  ...
  const timer=setTimeout(async()=>{
    ...
    try{
      const channel=await client.channels.fetch(channelId).catch(e=>{if(e.code===10003)return null;throw e;});
...
export async function reconcileRooms(client,store){
  for(const room of store.rooms()){
    let channel;
    try{channel=await client.channels.fetch(room.channel_id);}catch(e){
      if(e.code===10003){store.deleteRoom(room.channel_id);continue;}
      console.warn('Nie można sprawdzić pokoju; zachowano zapis:',e.message);continue;
    }
...
const ephemeral=(i,content)=>i.deferred?i.editReply({content,components:[],allowedMentions:{parse:[]}}):
...
export async function handleRoomInteraction(i,store){
  ...
  const room=store.room(id),channel=i.guild.channels.cache.get(id)||await i.guild.channels.fetch(id).catch(e=>{if(e.code===10003)return null;throw e;});
  ...
  }else if(!owner&&!isStaff){await denyPermission(i,'Właściciel pokoju lub Moderator','Nie możesz zmieniać ustawień tego pokoju.');return true;}
  ...
  try{
    ...
    if(i.isModalSubmit()&&action==='modal_rename'){
      ...
      await channel.permissionOverwrites.edit(i.client.user.id,{ViewChannel:true,Connect:true,SendMessages:true,ReadMessageHistory:true,EmbedLinks:true,AttachFiles:true},{reason:'Dostęp bota do panelu pokoju'});
