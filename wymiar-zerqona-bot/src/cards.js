...
export async function profileCard(user,row,rank,roleName='Początkujący'){
  ...
  const metrics=[['PORTFEL',`${fmt(row.balance)} ◈`,'Monety do wykorzystania','coin'],['BANK',`${fmt(row.bank||0)} ◈`,'Odłożone monety','bank'],['WIADOMOŚCI',fmt(row.messages),'Wiadomości nagrodzone XP','bolt'],['ROZMOWY VC',duration(row.voice_seconds),'Czas naliczony do XP','orbit']];
  ...
  text(ctx,'STAŁYCH PRZEDMIOTÓW',948,734,11,muted,true);line(ctx,948,756,1368,756);
...
function cashChart(ctx,x,y,w,h,days,c){
  ...
  text(ctx,`SKALA 0–${fmt(max)} ◈`,x+w,y-13,10,muted,false,w,'right');
...
export async function economyCard(user,row,rank,inventory=[],now=Date.now()){
  ...
  metric(ctx,1074,181,318,247,'CAŁY MAJĄTEK',fmt(wealth),'',c,'crystal');
  text(ctx,'STAŁE PRZEDMIOTY',1096,338,10,muted,true);text(ctx,String(countPermanent(inventory)),1096,390,34,c.bright,true);
  ...
  [['Zarobione nagrody',stats.earned||0],['Zakupy w sklepie',stats.spent||0],['Wynik gier',stats.games||0]].forEach(([label,value],n)=>{text(ctx,label,950,544+n*59,14,muted);text(ctx,`${fmt(value)} ◈`,1366,544+n*59,20,white,true,227,'right');if(n<2)line(ctx,950,562+n*59,1366,562+n*59);});
  text(ctx,`${stats.transactions||0} operacji od uruchomienia historii`,950,716,11,muted,false,412);
  const reward=[['/daily',ready(row.last_daily_at,config.economy.dailyCooldownMs,now),'350–650 ◈ • co 24 godziny'],['/praca',ready(row.last_work_at,config.economy.workCooldownMs,now),row.work_boost_until>now?'150–360 ◈ • kontrakt aktywny':'100–240 ◈ • co 45 minut'],['/weekly',ready(row.last_weekly_at,config.economy.weeklyCooldownMs,now),'1200 ◈ • co 7 dni']];
...
export async function topCard(rows,users,kind='xp',offset=0,total=rows.length){
  ...
  rows.forEach((row,n)=>{const y=191+n*78,user=users[n]||{name:'Użytkownik'},pos=offset+n+1;panel(ctx,48,y,1344,66,c,pos<=3);
    ...
    const val=kind==='xp'?`LV ${levelForXp(row.xp)}   /   ${fmt(row.xp)} XP`:kind==='voice_seconds'?duration(row.voice_seconds):kind==='messages'?`${fmt(row.messages)} WIAD.`:`${fmt(kind==='wealth'?row.balance+(row.bank||0):row[kind]||0)} ◈`;
...
export async function shopCard(user,row,inventory,selected='xp_boost',category='all',page=1){
  ...
  text(ctx,`${fmt(item.price)} ◈`,254,406,38,c.bright,true,351,'center');
  ...
  text(ctx,(shopCategories[category]||shopCategories.all).toUpperCase(),48,507,23,white,true);text(ctx,`PORTFEL ${fmt(row.balance)} ◈  •  STRONA ${page}/${pages}`,1392,506,15,c.bright,true,720,'right');
  pageItems.forEach(([id,it],n)=>{const x=48+(n%3)*458,y=537+Math.floor(n/3)*200,isSelected=id===selected;
    ...
    text(ctx,it.name,x+115,y+61,17,white,true,291);text(ctx,it.tag,x+115,y+88,10,muted,true,279);text(ctx,`${fmt(it.price)} ◈`,x+115,y+126,24,c.bright,true,270);
...
export async function inventoryCard(user,row,inventory){
  ...
  text(ctx,'TWOJA KOLEKCJA',48,149,32,white,true);text(ctx,`${countPermanent(rows)} STAŁYCH  /  ${rows.filter(r=>!permanent(items[r.item_id])).reduce((n,r)=>n+r.quantity,0)} BOOSTÓW`,1392,147,13,c.bright,true,700,'right');
  if(!rows.length){artifact(ctx,'crystal',230,285,80,c);text(ctx,'Zacznij od pierwszego przedmiotu.',403,279,26,white,true);text(ctx,'Otwórz /sklep i wybierz coś dla siebie.',404,319,19,muted);}
  rows.forEach((r,n)=>{const item=items[r.item_id],x=48+(n%3)*458,y=183+Math.floor(n/3)*143;panel(ctx,x,y,428,124,c);artifact(ctx,item.icon,x+53,y+60,31,c);text(ctx,item.name,x+103,y+39,16,white,true,299);text(ctx,permanent(item)?'STAŁY PRZEDMIOT':`${r.quantity} SZT. • ${duration(item.durationMs/1000)}`,x+103,y+68,11,c.accent,true,295);
...
export async function bankCard(user,row){
  ...
  panel(ctx,48,123,1344,430,c,true);artifact(ctx,'bank',223,326,109,c);text(ctx,'TWÓJ SKARBIEC',406,181,16,c.bright,true);fit(ctx,`${fmt(row.bank||0)} ◈`,401,277,70,912);
  text(ctx,`Portfel: ${fmt(row.balance)} ◈`,407,328,24,white,true);text(ctx,`Cały majątek: ${fmt(wealth)} ◈`,407,368,20,muted);
...
export async function achievementsCard(user,row,status){
  ...
  status.forEach((a,n)=>{const x=48+(n%2)*686,y=188+Math.floor(n/2)*175;panel(ctx,x,y,658,151,c,a.ready);artifact(ctx,a.claimed?'crown':'crystal',x+57,y+69,32,c);
    ...
    text(ctx,a.claimed?'ODEBRANO':a.ready?'GOTOWE DO ODBIORU':'W TRAKCIE',x+112,y+131,11,a.ready?c.bright:muted,true,290);text(ctx,`+${fmt(a.reward)} ◈`,x+629,y+131,15,c.bright,true,250,'right');
  });return png(cv);
}
